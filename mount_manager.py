...
class MountManager:
    def __init__(self, mount_point, mount_type=None, device=None, remote_path=None, server=None, user=None, password=None, credential_file=None, port=None, secure=None):
        ...
        self.__logger = logging.getLogger("MountManager")
        ...
        try:
            if not self.__device and not self.__mount_type:
                ...
                if self.__is_in_fstab():
                    self.__logger.info(f'Mounting {self.__mount_point} from /etc/fstab.')
                    subprocess.run(['mount', self.__mount_point])
                else:
                    self.__logger.info(f"Device and type not specified, and {self.__mount_point} is not in /etc/fstab.")

            elif not self.__mount_type:
                # Attempt to mount the device on the mount point directly
                self.__logger.info(f'Mounting {self.__mount_point} filesystem from device: {self.__device}.')
                ...
                self.__logger.info(f'Mounting {self.__mount_type} share {self.__mount_point} from {self.__server}.')
    ...
    def umount(self):
        if not self.__is_mounted():
            self.__logger.info(f"Filesystem at {self.__mount_point} is not mounted.")
            return
    ...
    def __is_in_fstab(self):
        try:
            ...
            self.__logger.info(f"Error reading /etc/fstab: {str(e)}")
            return False
